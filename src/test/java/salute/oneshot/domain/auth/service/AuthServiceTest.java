...
class AuthServiceTest {
    ...
    void successLogIn() {
        ...
        User user = UserTestFactory.createUser();
        TokenInfo toKenInfo = TokenTestFactory.createAccessTokenInfo();
    ...
    void successLogOut() {
        ...
        given(userRepository.existsUserById(sDto.getId()))
                .willReturn(true);
        ...
        // then
        then(userRepository).should().existsUserById(sDto.getId());
    ...
    @DisplayName("로그아웃 실패: 존재하지 않는 메일로 로그인")
    @Test
    void notFoundEmailLogOut() {
        // given
        LogOutSDto sDto = UserTestFactory.createLogOutSDto();
        given(userRepository.existsUserById(sDto.getId()))
                .willReturn(false);
        ...
        then(userRepository).should().existsUserById(sDto.getId());
        then(jwtProvider).shouldHaveNoInteractions();
        then(blacklistRepository).shouldHaveNoInteractions();
    }
