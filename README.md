...
<details>
  <summary>인기 칵테일 선정 시 데이터 유실 문제 해결 </summary>

### 문제
- 일정 주기마다 Redis ZSet을 기준으로 인기 칵테일을 선정한 뒤, 점수를 초기화함</br>
- 이 과정에서 선정과 초기화 사이에 발생하는 조회/좋아요 이벤트가 반영되지 않아 데이터 유실이 발생 가능성 확인</br>


### 원인
- 선정과 초기화 사이에 발생하는 조회/좋아요 이벤트가 reset으로 인해 사라져버림</br>

### 해결
```redisTemplate.rename(RedisConst.COCKTAIL_SCORE_KEY, RedisConst.COCKTAIL_SCORE_SNAPSHOT_KEY);```</br>
- 메인키의 이름을 변경하여 특정 시점까지의 통계를 가져옴.</br>
- 이후 선정 로직은 backup된 키 기준으로 인기 칵테일 선정.</br>
- 메인 키는 rename 이후 해당 키에 대한 명령어 실행시 자동 생성되므로 새로운 이벤트를 바로 수용 가능.</br>
- Redis 내부에서 RENAME 연산은 원자적이므로 동시성 이슈 없음.</br>
</details>
